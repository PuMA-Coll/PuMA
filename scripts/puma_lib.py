...
class Observation(object):
	...
	self.DEFAULT_PAR_DIRNAME = '/opt/pulsar/tempo/tzpar/'
	...
	def do_rfi_search(self):
		...
		if self.params2reduc['reuse']:
			...
			subprocess.check_call(rfifind, cwd=self.path_to_folder)
			selfmaskname = output + '_rfifind.mask'
	...
	def prepare_prepfold_cmd(self):
		...
		prepfold_args = ['prepfold',
				...
				'-mask', self.params2reduc['maskname'],
		...
		if self.params2reduc['ftype'] == 'timing':
			prepfold_args.extend(('-timing', self.params2reduc['dotpar']))
		elif self.params2reduc['ftype'] == 'par':
			prepfold_args.extend(('-par', self.params2reduc['dotpar'],
	...
	def set_params2reduc(self, ftype='timing', path_to_dir=os.environ['PWD'], par_dirname=self.DEFAULT_PAR_DIRNAME, ptopo=1.0, ncores=1):
		...
		configfile.read(configdest + self.pname + '.ini')
		...
		self.params2reudc['reuse'] = configfile.getboolean('rfi', 'reuse')

		dotpar_filename = self.par_dirname + '/' + self.pname + '.par'
		if os.path.isfile(dotpar_filename) is False:
	...
	def do_glitch_search(self, path_to_dir=self.path_to_dir, par_dirname=self.par_dirname, ncores=1):
