...
def plot_waterfall(data, start, duration, 
    ...
    fig = plt.figure(figsize=(7,6)) # buen tamao para hacer pltos
#    fig = plt.figure(figsize=(3,6))
    ...
    #ax_im.axvline(x=-0.0005, color='red', linestyle='-', linewidth=2)
    ax_im.xaxis.get_major_formatter().set_useOffset(False)
    ax_im.set_xlabel("Time from Center [s]", fontsize=17)
    ax_im.set_ylabel("Frequency [MHz]", fontsize=17)
    #ax_im.set_xticks(np.arange(-0.02, 0.021, 0.01))

    ax_im.tick_params(labelsize=17)
    #ax_im.set_xticklabels(["-0.02", "-0.01", "0.00", "0.01", "0.02"], fontsize=14)
    #ax_im.set_xticklabels(fontsize=14)
    ...
    if integrate_ts:
        ...
        ax_ts.tick_params(axis='x', labelsize=14, labeltop=False)
        ax_ts.tick_params(axis='y', labelleft=False)

        #ax_ts.yaxis.set_label('A.U.')
        ...
        #print("delta_t:", delta_t)
         ...
         # Crear texto
        central_time_text = "$\\mathrm{Center:}$" + "{:.2f}".format(float(time_center)) + "$\\mathrm{s}$"
#       central_time_text = u"$\\mathrm{Center:}$" + str(time_center) + u"$\\mathrm{s}$"
        ...
        ax_ts.text(0.25, 0.65, central_time_text + "\n" + delta_t_text,
           ha='center', va='center', transform=ax_ts.transAxes,
           fontsize=15, color="black", bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))
    ...
    if output_filename:
        fig.savefig(output_filename+'.png', bbox_inches='tight',dpi=300)
        fig.savefig(output_filename+'.pdf', bbox_inches='tight',dpi=300)
